{
  ...
  outputs = { self, nixpkgs }: 
    ...
    in {
      overlays.default = final: prev: { dwc = self.dwc final; neuwld = self.neuwld final; neuswc = self.neuswc final; 
          fixed-libdrm = (final.libdrm.dev.overrideAttrs {
                postInstall = ''
      ...
      packages = forAllSystems nixpkgs.legacyPackages (system: pkgs: rec {
          ...
          neuwld = self.neuwld pkgs;
          });
      dwc = pkgs: pkgs.stdenv.mkDerivation rec {
        name = "dwc";
        src = pkgs.fetchgit {
      ...
      neuswc = pkgs: pkgs.stdenv.mkDerivation rec {
        name = "neuswc";
        src = pkgs.fetchgit {
    ...
    neuwld = pkgs: pkgs.stdenv.mkDerivation rec {
      name = "neuwld";
      src = pkgs.fetchgit {
